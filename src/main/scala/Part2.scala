...
object Part2 extends App {
  ...
  val counter = CachedAndCountedFunction(function.tupled)
  val countedFunction: (Double, Double) => Double = Function.untupled(counter.getFunc)

  val resultTable = Seq.tabulate(50)(i => pow(1.25, - i)).map { epsilon =>
